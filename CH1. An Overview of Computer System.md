...
- **하드웨어(hard)**: 컴퓨터 정보의 전송 통로. 정보 처리가 실제로 일어나게 해주는 <u>물리적인 실체들</u>.
...
- 컴퓨터의 주요 구성 요소들: 중앙처리장치(CPU), 기억장치(Memory), 입출력 장치(I/O devices), 시스템 버스(이 장치들을 연결한다)
...
- CPU가 처리할 프로그램 코드와 데이터를 저장한다.

- 기억장치의 분류: 특성, 속도 및 용도에 따라 (1) 주기억장치 (2) 보조저장장치로 나눈다.
...
- 단어는 CPU에 의해 한 번에 처리될 수 있는 비트들의 그룹이다.
...
- **시스템 버스(system bus)**
  ...
  - 시스템 버스의 구성: (1) **주소 버스(address bus)**, **데이터 버스(data bus)**, **제어 버스(control bus)**
- **주소 버스(address bus)**
...
- **기억장치 쓰기 시간(memory write time)**
  - CPU가 기억장치로 데이터로 주소와 데이터를 보낸 순간부터
  - 기억장치에 저장이 완료되 때까지의 시간
