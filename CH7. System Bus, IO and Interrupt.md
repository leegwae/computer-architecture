...
<img src=".\Images\7-2.PNG" alt="7-2" style="zoom:50%;" />
...
<img src=".\Images\7-11.PNG" alt="7-11" style="zoom:60%;" />
...
<img src=".\Images\7-16.PNG" alt="7-16" style="zoom:60%;" />
...
- 폴링 방식, 인터럽트 방식의 경우, 데이터 전송할 때 CPU가 관여하고, 특히 폴링 방식의 경우 CPU가 항상 상태 레지스터를 검사해야 한다.
- **DMA(Direct Memory Access)**: <u>CPU의 개입 없이</u> I/O 장치와 기억장치 사이에 데이터를 전송하는 방식
  - **사이클 스틸링(cycle stelaing)**: CPU가 주기억장치를 액세스하지 않는 시간을 이용하기 때문.
- 방법: CPU가 주기억장치를 액세스하지 않는 시간(CPU가 내부적으로 명령어를 해독하거나 ALU 연산을 수행하는 시간) 동안에 시스템 버스를 사용하여 주기억장치와 I/O 장치 간의 데이터 전송을 수행
- **DMA 제어기(DMA controller)**: 메인 메모리에서 I/O 장치로의 입출력만을 담당하는 일종의 CPU

<img src=".\Images\7-20.PNG" alt="7-20" style="zoom:50%;" />

- 동작 순서
  - (1) CPU가 DMA 제어기(DMA controller)로 다음의 정보를 포함한 명령을 전송
  ...
  - (4) DMA 제어기가 주기억장치로부터 데이터를 읽어서, 디스크 제어기로 전송(디스크에 저장). 
    - 각 데이터에 대하여 ②,③,④번을 두 번 반복(시스템 버스를 두 번 사용)
      - 주기억장치 -> DMA 제어기
      - DMA 제어기 -> 디스크 제어기 -> 데이터 저장
  - 저장할 데이터들이 남아있다면, 그 수만큼 ②~④번 동작을 반복
  - 모든 데이터들의 저장이 완료되면, CPU로 INTR 신호를 전송

- DMA의 장점: CPU가 전체 입출력에 관여하지 않고 다른 일을 수행할 시간을 확보할 수 있다.
- DMA의 문제점: 시스템 버스를 두 번 사용하므로 버스 사용량 증가로 성능 저하
  - 해결책: I/O 장치를 DMA에 접속하기
    - DMA 제어기가 주기억장치를 액세스할 때만 시스템 버스 사용하도록
